...
function registerCmd(
  ...
  cmdName: string,
  cmdCallback: (...args: any[]) => any
) {
  context.subscriptions.push(
    vscode.commands.registerCommand(`${EXTENSION_NAME}.${cmdName}`, cmdCallback)
  );
}
...
async function parseDprFiles(dprFilePath: string) {
  ...
  return (
    (await fs.promises.readFile(dprFilePath, 'utf8'))
      ?.match(/(?<=in \')[^\']+(?=\')/gm)
...
async function runDelphi() {
  ...
  fs.promises
    .access(dprojPaths.exe, fs.constants.X_OK)
...
async function runMSBuildProcess(
  dprojPath: string,
...
async function parseDprojPaths(dprojFilePath: string): Promise<DprojPaths | undefined> {
  ...
  for (const propGroup of propGroups) {
    ...
    exe = path.isAbsolute(exePath) ? exePath : path.join(path.dirname(dprojFilePath), exePath);
  }
...
function checkExtension(extensionID: string, description: string, button: string) {
  if (!vscode.extensions.getExtension(extensionID)) {
    vscode.window.showInformationMessage(description, button).then((selection) => {
      if (selection === button) {
